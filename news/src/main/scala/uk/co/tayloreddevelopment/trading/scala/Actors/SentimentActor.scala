...
case class GetSentiment(tag:String,text:String, link:String, source:String)
...
val notificationActor: ActorRef = this.context.actorOf(Props[NotificationActor])
  override def receive: Receive = {
    case x: GetSentiment =>
      notificationActor ! NotifySentiment(x.tag,x.text,x.source,sentiment.getSentiment(x.text),x.link)
